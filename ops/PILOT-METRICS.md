...
| --- | --- | --- |
| Sentry | Error and exception reporting, DSN-gated (`config/settings/prod.py:299-321`), scrubbers pinned by `tests/security/test_credential_log_hygiene.py`. Four explicit capture sites exist in the tree: invitation send failures (`apps/accounts/views.py:179`), LGPD notification failures (`apps/lgpd/views.py:84`), storage inconsistencies (`apps/portal/views.py:474`), rate-limit-store outages (`apps/security/ratelimit.py:196`). Commit `74b3bef` also added `CeleryIntegration` for unhandled worker/beat task exceptions. | **Code exists. Operator configuration remains:** reporting begins once `SENTRY_DSN` is set on the target host. |
...
| Healthchecks.io | Dead-man checks for the nightly backup, off-host copy, and daily live verification. Silence is the signal. | **Code exists; external checks and URLs do not.** Commit `0606871` added the backup check's `/start`, success-after-freshness-marker, and `on_exit` `/$rc` failure pings. Commit `bf453f6` added a separate `HC_OFFHOST_URL` with `/start`, success, and `/$rc` failure pings around the two `aws s3 cp` operations; off-host-copy failures use documented exit code `5`. Commit `74b3bef` added the scheduled `verify-live` workflow, which sends success or `/fail` through `HC_VERIFY_URL`. The operator still must create `app-mei-backup` and `app-mei-offhost-copy`, set their grace to 2h, place `HC_URL` and `HC_OFFHOST_URL` in `/etc/app-mei/backup.env` (loaded by the unit's optional `EnvironmentFile=-`), create `app-mei-verify-live`, and add its ping URL as the `HC_VERIFY_URL` repository secret. |
...
**Healthchecks.io is not a source for any metric row**; its three checks are alerting
surfaces. The code side is wired: `ops/backup.sh` emits the local-backup and independent
off-host signals, and `.github/workflows/verify-live.yml` emits the live-verification
signal. The operator side is not: the checks and their URLs have not been created or
configured. Until that happens, working backup health is read from `/healthz`'s
`"backup"` field and the systemd unit state, not from a Healthchecks dashboard.
...
| Document loss or checksum mismatch | A download returning bytes whose sha256 does not match the stored checksum; a storage-inconsistency report in Sentry (`apps/portal/views.py:474`); a document row whose object is absent from the bucket. | Pause invitations and pause uploads. Do not delete anything. Attempt recovery from object-storage versioning, then from backup per `ops/RESTORE.md`. **The off-host copy code exists** (`bf453f6`), but no real remote bucket or credential is configured; `OFFHOST_S3_BUCKET` remains its activation switch. Until the operator activates it, recovery remains limited to the local backup tree and object-storage versioning. Record which documents were affected and inform the firm. |
...
| Unrecoverable backup failure longer than 48 hours | **The currently active signals are `/healthz` reporting `"backup": "stale"`** (the freshness marker, stamped only on full local success, ages out after 36 hours) plus the systemd unit's state on the box. The code-side Healthchecks signals exist (`0606871`, `bf453f6`), but the operator has not created the checks or configured `HC_URL` / `HC_OFFHOST_URL`, and no real off-host bucket is active. Once configured, local-check silence or `/$rc`, off-host-check silence or `/5`, and an absent remote object join this row. | Pause invitations. A pilot without a recoverable backup does not continue. Run the backup by hand, and if it cannot be made to succeed, stop the pilot until it can. |
