"""What the law says: effective-dated parameters, due rules, and the calendar.

All three tables are platform-level, not tenant-scoped, and that is deliberate.
The MEI ceiling and whether 20 November is a holiday are facts about Brazilian law,
not about any one accounting firm. A per-tenant copy would let two firms hold
contradictory answers about the same statute and would make every firm rediscover
the same regulatory change independently. All three are listed in
`apps.core.rls.NON_TENANT_TABLES`; the coverage meta-test fails the build if a table
is neither tenant-scoped nor listed there.
...
changes -- the day the firm can least afford one -- so every number the engine reads
is a row here and superseding it is a pure INSERT.
"""
