...
| NEW (this plan): storage lifecycle unadjudicated (orphan bytes, no real-bucket proof, single bucket, on-host backups, no object DR) | EVIDENCE RECORDED | PILOT-203/204/205/301/303 + §29 records what remains. | The shipped storage boundary is configured in `config/settings/prod.py`; restore-drill evidence is recorded in `.evidence/PILOT2-302H-operator.txt`, `.evidence/PILOT2-303-operator.txt`, and `.evidence/PILOT2-405-operator.txt`. |
