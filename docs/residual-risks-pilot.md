...
- **RECORDED**, **EVIDENCE RECORDED**, **RESOLVED**, **CLOSED ON THE TARGET** or
  **OBSERVED LIVE** when the external evidence exists. Those cells cite the operator
  artifact, under the execution worktree's local `.evidence/` root, that holds it.
...
| §2.3 Docker log retention | OPERATOR GATE | PILOT-209 verifies/configures host rotation (driver, max-size/max-file, journald caps, permissions, post-rotation canary absence) on current AND target VPS (re-run in PILOT-402). | **CLOSED ON THE TARGET.** `docker-compose.prod.yml` defines the shipped services; retention is host-driver state. On the Lightsail host every application container reports `json-file` at 50m x 5, a 300 MiB flood left five rotated files, journald is capped at 500M, and a post-rotation credential sweep found zero matches with a positive control: `.evidence/PILOT2-209-operator.txt`, configuration re-read at Phase B in `.evidence/PILOT2-402-operator.txt`. The OCI host was never verified and is now stopped. |
...
| §6.2 Staging release inferred | MITIGATE BEFORE PILOT | PILOT-002 observes now; PILOT-202 makes it observable forever (/versionz + public deploy assert). | **LANDED AND OBSERVED LIVE.** The mechanism: `versionz` is served from `apps/core/views.py` and routed in `config/urls.py`, `.github/workflows/ci.yml` asserts it over the public edge against the deployed SHA, and `.github/workflows/verify-live.yml` re-checks it against the head of `main` on a schedule. The first Lightsail push deploy asserted `/versionz` equal to its SHA (`.evidence/PILOT2-403-happy.txt`), the public edge served the freeze SHA unpinned after the flip (`.evidence/PILOT2-404O2-operator.txt`), a scheduled `verify-live` run passed against the target, and UptimeRobot M3 watches the deployed release (`.evidence/PILOT2-406-operator.txt`). |
...
| NEW (found in review of PILOT-207): Celery worker events widen Sentry's stack-local capture | ACCEPTED AND RECORDED; owner decides at PILOT-504 | Adding `CeleryIntegration` extends capture from request handling to task execution, so `include_local_variables` (SDK default: on) can now serialize task-local objects — a `client` during a failed calendar sweep, for example — that no web request would have held. The existing denylist, both send hooks and `send_default_pii=False` still apply to every one of those events, which bounds the exposure rather than removing it. At 10-30 pilot clients the accepted view is that diagnostics on a broken worker are worth more than the residue. | **RESOLVED.** PILOT-504 set `include_local_variables=False` in `SENTRY_OPTIONS` in `config/settings/prod.py`, which drops frame locals from every event, worker events included; the non-vacuous control is `tests/security/test_sentry_frame_locals.py`, and the scrubbing canaries stay in `tests/security/test_credential_log_hygiene.py`. PILOT2-107T confirmed it on the Lightsail target: a controlled event arrived with the deployed release and no frame locals (`.evidence/PILOT2-107T-operator.txt`). That event came from the web container; no worker-origin event was captured, and the setting is process-wide, so none was needed to close this row. |
| NEW (this plan): storage lifecycle unadjudicated (orphan bytes, no real-bucket proof, single bucket, on-host backups, no object DR) | EVIDENCE RECORDED | PILOT-203/204/205/301/303 + §29 records what remains. | **RECORDED.** The shipped storage boundary is configured in `config/settings/prod.py`. The documents stay in the OCI bucket under the keep-oci ruling, so no storage setting was renamed (`.evidence/PILOT2-405M-operator.txt`). Versioning, the 30-day previous-version rule, the real-bucket probe and a byte-verified version restore: `.evidence/PILOT2-405-operator.txt`. Off-host backup copies armed with a separate reader: `.evidence/PILOT2-301-operator.txt`. Restore drills: `.evidence/PILOT2-302L-operator.txt`, `.evidence/PILOT2-302H-operator.txt`, `.evidence/PILOT2-303-operator.txt`. From the target: the scheduled nightly of 2026-09-24 copied off-host and a separate reader matched its SHA-256 (`.evidence/PILOT2-406-operator.txt`). |
...
**Resolution.** The decision went to privacy. PILOT-504 set `include_local_variables=False`
in production, pinned by `tests/security/test_sentry_frame_locals.py`, and PILOT2-107T saw
a target event arrive with no frame locals (`.evidence/PILOT2-107T-operator.txt`). Worker
tracebacks now carry frames without values. That's the accepted cost.
